...
const myChaincodeName = "auditor";

const memberAssetCollectionName = "auditCollection";
...
async function main() {
  try {
    ...
    contractOrg1.addDiscoveryInterest({
      ...
      collectionNames: [memberAssetCollectionName],
    });
    ...
    try {
      ...
      */
      ...
      console.log("\n**************** As Org1 Client ****************");
      console.log(
        "Adding Assets to work with:\n--> Submit Transaction: CreateAudit " +
          assetID1
      );
      let statefulTxn = contractOrg1.createTransaction("CreateAudit");
      ...
      statefulTxn.setTransient({
        audit_properties: tmapData,
      });
    //   result = await statefulTxn.submit();
      console.log("result1: ", result);

      // Add asset2
      console.log("\n--> Submit Transaction: CreateAudit " + assetID2);
      statefulTxn = contractOrg1.createTransaction("CreateAudit");
      ...
      console.log("\n--> Evaluate Transaction: QueryAudits asset0-asset9");
      ...
      if (!result || result.length === 0) {
        doFail("recieved empty query list for QueryAudits");
      }
      return;
      console.log(
      ...
      console.log(
        "\n--> Evaluate Transaction as Org2: ReadAssetPrivateDetails " +
          assetID1 +
          " from " +
          org2PrivateCollectionName
      );
      result = await contractOrg2.evaluateTransaction(
        "ReadAssetPrivateDetails",
        org2PrivateCollectionName,
        assetID1
      );
